-- Quest approval now feeds the active companion's xp alongside the hero's.
-- (Full recreate of award_submission; the only change is the companions
-- update after the profiles update.)
...
begin
  ...
  -- the bonded companion grows with every quest completed together
  update public.companions set xp = xp + t.xp_reward
